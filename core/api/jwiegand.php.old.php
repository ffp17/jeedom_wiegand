...
if($cmd == "tag")
{
    ...
    $elogicReader->setConfiguration('BadgerID',$value);
    $elogicReader->save();
    $cmd = jwiegandCmd::byEqLogicIdCmdName($elogicReader->getId(),'BadgerID');
    $cmd->event($value);
    if (!is_object($elogic)) {
...
if($cmd == "pin")
{
	...
	else  
	{
		...
		$cmd = jwiegandCmdCmd::byEqLogicIdCmdName($elogic->getId(),'BadgerID');
		if (!is_object( $cmd )){
